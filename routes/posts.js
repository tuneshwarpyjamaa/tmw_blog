...
router.post('/create', requireAuth, [
    ...
    await pool.query('INSERT INTO posts (title, content, category, author_id, status, published_at) VALUES ($1, $2, $3, $4, \'published\', CURRENT_TIMESTAMP)', [title, sanitizedContent, category, req.session.userId]);
    res.redirect('/posts');
});
...
router.post('/:postId/edit', requireAuth, [
    body('title').isLength({ min: 1, max: 200 }).withMessage('Title must be 1-200 characters').trim().escape(),
    body('content').isLength({ min: 1 }).withMessage('Content is required').trim(),
    body('category').isLength({ min: 1, max: 50 }).withMessage('Category must be 1-50 characters').trim().escape()
], async (req, res) => {
    ...
    const { title, content, category } = req.body;
    const sanitizedContent = xss(content); // Sanitize HTML content
    await pool.query('UPDATE posts SET title = $1, content = $2, category = $3, updated_at = CURRENT_TIMESTAMP WHERE post_id = $4 AND author_id = $5', [title, sanitizedContent, category, req.params.postId, req.session.userId]);
